...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif (strlen($serial) !== 14 || !ctype_digit($serial)) {
        $mensaje = "<span style='color:#d8000c;'>El número de serie debe tener exactamente 14 dígitos. Ejemplo válido: 12506910800703</span>";
    } else {

        if (isset($conn)) {
        
            $sql = "UPDATE [ADCS_CHI].[dbo].[mac_ids] SET mac_status='L' WHERE uut_id = ? AND mac_status = 'P'";
            $params = array($serial);
            ...
            if ($stmt === false) {
                ...
                if ($rows === false) {
                    $errors = print_r(sqlsrv_errors(), true);
                    $mensaje = "<span style='color:#d8000c;'>No se pudo obtener filas afectadas: <pre>$errors</pre></span>";
                } elseif ($rows > 0) {
                    $mensaje = "Etiqueta con número de serie <b>" . htmlspecialchars($serial) . "</b> lista para reimprimir. ";
                } else {
                    $mensaje = "<span style='color:#d8000c;'>No se actualizó ningún registro. El UUT_ID puede no existir o su estado no era 'P'.</span>";
                }
...
<div class="container">
    <h2>Reimprimir etiqueta Welcome Scan Here</h2>
